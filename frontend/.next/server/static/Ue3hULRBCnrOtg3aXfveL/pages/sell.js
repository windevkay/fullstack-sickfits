  ...
  function CreateItem() {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "handleChange", function (e) {
      var _e$target = e.target,
          ...
          type = _e$target.type,
          name = _e$target.name; //all inputs come in as string, so if its a number we expect then convert it
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                files = e.target.files; //use javascript form data api to prep the data 

                data = new FormData();
                data.append('file', files[0]); //add the upload preset required by cloudinary 

                data.append('upload_preset', 'sickfits'); //call the cloudinary api

                _context.next = 6;
                return fetch('https://api.cloudinary.com/v1_1/dw8yxwteb/image/upload', {
                  method: 'POST',
                  body: data
                });
